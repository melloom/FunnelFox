...
## Recent Changes
- 2026-02-07: Redesigned Account Settings page with comprehensive subscription panel. Shows usage progress bars (discoveries, leads), plan details grid (status, billing date, payment method), status badges (Active/Canceling/Admin), cancel/resume via Stripe billing portal. Admin users see "Permanent Pro access". Upgrade CTA for free users. Enhanced /api/subscription to return Stripe metadata (billing cycle, period dates, card info, cancel status, lead count). Files: client/src/pages/account.tsx, server/stripe-routes.ts.
...
- 2026-02-07: Added Account Settings page (/account) with profile info, subscription status, and account deletion. Delete requires typing "DELETE" to confirm. Active Pro subscribers must cancel subscription first. Admin accounts bypass subscription check. Stripe API failures block deletion (503). DELETE /api/account endpoint. Account link added to sidebar. Files: client/src/pages/account.tsx, server/replit_integrations/auth/routes.ts.
